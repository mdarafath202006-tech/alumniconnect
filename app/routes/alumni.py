"""
app/routes/alumni.py – Alumni blueprint.
"""
...
def dashboard():
    ...
    try:
        ...
        if profile:
            ...
            cur.execute("SELECT COUNT(*) AS cnt FROM mentorship_requests WHERE alumni_id=%s AND status='accepted'", (profile["id"],))
...
def profile():
    ...
    try:
        if request.method == "POST":
            cur.execute("""
                UPDATE alumni SET company=%s, job_role=%s, skills=%s,
                location=%s, linkedin=%s, bio=%s WHERE user_id=%s
            """, (
...
def respond_request(req_id, action):
    ...
    cur = db.cursor()
    try:
        cur.execute("UPDATE mentorship_requests SET status=%s WHERE id=%s", (status, req_id))
        db.commit()
    finally:
        cur.close(); db.close()
